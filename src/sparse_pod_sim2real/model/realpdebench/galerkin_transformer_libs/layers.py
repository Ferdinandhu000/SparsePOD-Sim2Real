...
class SpectralConv1d(nn.Module):
    def __init__(self, in_dim,
                 ...
                 modes_x=None,  # number of fourier modes in x dimension
                 modes_y=None,  # number of fourier modes in y dimension
                 modes_t=None,  # number of fourier modes in t dimension
                 n_grid=None,
                 dropout=0.1,
                 return_freq=False,
                 activation='silu',
...
class SpectralConv3d(nn.Module):
    def __init__(self, in_dim,
                 out_dim,
                 modes_x: int,  # number of fourier modes in x dimension
                 modes_y: int,  # number of fourier modes in y dimension
                 modes_t: int,  # number of fourier modes in t dimension
                 ):
        ...
        self.modes1 = modes_t #Number of Fourier modes to multiply, at most floor(N/2) + 1
        self.modes2 = modes_x
        self.modes3 = modes_y

        self.scale = (1 / (in_dim * out_dim))
        self.weights1 = nn.Parameter(self.scale * torch.rand(in_dim, out_dim, self.modes1,
                                    self.modes2, self.modes3, dtype=torch.cfloat))
        self.weights2 = nn.Parameter(self.scale * torch.rand(in_dim, out_dim, self.modes1,
                                    self.modes2, self.modes3, dtype=torch.cfloat))
        self.weights3 = nn.Parameter(self.scale * torch.rand(in_dim, out_dim, self.modes1, 
                                    self.modes2, self.modes3, dtype=torch.cfloat))
        self.weights4 = nn.Parameter(self.scale * torch.rand(in_dim, out_dim, self.modes1,
                                    self.modes2, self.modes3, dtype=torch.cfloat))
