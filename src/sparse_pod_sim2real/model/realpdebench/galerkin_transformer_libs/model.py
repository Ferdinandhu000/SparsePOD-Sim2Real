from realpdebench.model.galerkin_transformer_libs.layers import *
...
class SpectralRegressor(nn.Module):
    def __init__(self, in_dim,
        ...
        if self.spacial_fc:
            ...
            #  self.fc = nn.Linear(7, n_hidden)
        self.spectral_conv = nn.ModuleList([spectral_conv(in_dim=n_hidden,
                                                          out_dim=freq_dim,
                                                          modes_x=modes_x,
                                                          modes_y=modes_y,
                                                          modes_t=modes_t,)])
        self.convs = nn.ModuleList([nn.Conv3d(n_hidden, freq_dim, 1)])
        self.bns = nn.ModuleList([nn.BatchNorm3d(freq_dim)]) 
        for _ in range(num_spectral_layers - 1):
            ...
            self.convs.append(nn.Conv3d(freq_dim, freq_dim, 1))
            self.bns.append(nn.BatchNorm3d(freq_dim))

        self.n_grid = n_grid  # dummy for debug
        # self.dim_feedforward = default(dim_feedforward, 2*spacial_dim*freq_dim)
        self.dim_feedforward = default(dim_feedforward, 128)
        # self.regressor = nn.Sequential(
        #     nn.Linear(freq_dim, self.dim_feedforward),
        #     self.activation,
        #     nn.Linear(self.dim_feedforward, out_dim),
        # )
        self.regressor1 = nn.Linear(freq_dim, self.dim_feedforward)

        self.regressor2 = nn.Linear(self.dim_feedforward, out_dim)
    ...
    def forward(self, x, edge=None, pos=None, grid=None):
        ...
        1D:
            ...
            Output: (-1, n, n_targets)
        3D:
            Input: (-1, n, n, t, in_features)
            Output: (-1, n, n, t, n_targets)
        '''
        ...
        x = x.permute(0, 4, 1, 2, 3)  # (batch, x, y, t, in_channel) -> (batch, in_channel, t, x, y)
        x = F.pad(x, [0, self.padding, 0, self.padding, 0, self.padding])
        
        for i in range(self.num_spectral_layers):
            x1 = self.spectral_conv[i](x)
            x2 = self.convs[i](x)
            x = x1 + x2
            x = self.bns[i](x)
            if i < self.num_spectral_layers - 1:
                x = F.gelu(x)        
                
        x = x[..., :-self.padding, :-self.padding, :-self.padding]
        x = x.permute(0, 2, 3, 4, 1)  # (batch, in_channel, t, x, y) -> (batch, t, x, y, out_dim)
